...
geode::Result<std::vector<float>> readAudioFile(const char* filename, int targetSampleRate, AVSampleFormat targetSampleFormat, AVCodecParameters* outCodecParams)
{
    ...
    // Parameters must be copied into the context BEFORE opening the codec
    ...
    while (av_read_frame(formatContext, packet) >= 0) {
        if (packet->stream_index == audioStreamIndex && avcodec_send_packet(codecContext, packet) == 0) {
            while (avcodec_receive_frame(codecContext, frame) == 0) {
                int converted = swr_convert(swr, reinterpret_cast<uint8_t**>(convertBuffer), 4096,
                ...
                if (converted < 0) {
                    ...
                    avcodec_free_context(&codecContext);
                    avformat_close_input(&formatContext);
                    return geode::Err("Failed to convert audio frame: " + ffmpeg::utils::getErrorString(converted));
                }
...
geode::Result<std::vector<float>> resampleAudio(std::span<float> inputAudio, int inputSampleRate, int targetSampleRate) {
    ...
    int ret = swr_alloc_set_opts2(&swrCtx, &ch_layout, AV_SAMPLE_FMT_FLT,
...
geode::Result<> AudioMixer::mixVideoRaw(const std::filesystem::path& videoFile, std::span<float> raw, const std::filesystem::path& outputMp4File) {
    ...
    outputAudioStream->codecpar->format     = AV_SAMPLE_FMT_FLTP;
    ...
    if (ret = avcodec_parameters_to_context(audioEncCtx, audioCodecParams); ret < 0) {
    ...
    audioEncCtx->sample_fmt  = audioCodec->sample_fmts ? audioCodec->sample_fmts[0] : AV_SAMPLE_FMT_FLTP;
    audioEncCtx->time_base   = AVRational{1, static_cast<int>(sampleRate)};

    if (outputFormatContext->oformat->flags & AVFMT_GLOBALHEADER)
        audioEncCtx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;

    ret = avcodec_open2(audioEncCtx, audioCodec, nullptr);
    if (ret < 0) {
        avcodec_free_context(&audioEncCtx);
        avformat_free_context(outputFormatContext);
        avformat_close_input(&videoFormatContext);
    ...
    avcodec_parameters_from_context(outputAudioStream->codecpar, audioEncCtx);
    ...
    for (size_t i = 0; i < resampled.size(); i += frameSize * channels) {
        ...
        if (ret = avcodec_send_frame(audioEncCtx, audioFrame); ret < 0) {
            av_packet_free(&audioPacket);
            av_frame_free(&audioFrame);
            avcodec_free_context(&audioEncCtx);
            avio_closep(&outputFormatContext->pb);
        ...
        while (true) {
            int recv_ret = avcodec_receive_packet(audioEncCtx, audioPacket);
            if (recv_ret == 0) {
                av_packet_rescale_ts(audioPacket, audioEncCtx->time_base, outputAudioStream->time_base);
