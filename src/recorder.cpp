    ...
    while ((codec = av_codec_iterate(&iter))) {
        if (codec->type == AVMEDIA_TYPE_VIDEO &&
                (codec->id == AV_CODEC_ID_H264 || codec->id == AV_CODEC_ID_HEVC ||
                 ...
                 codec->id == AV_CODEC_ID_AV1  || codec->id == AV_CODEC_ID_MPEG4) &&
                avcodec_find_encoder_by_name(codec->name) != nullptr &&
                codec->pix_fmts &&
                std::ranges::find(vec, std::string(codec->name)) == vec.end())
...
geode::Result<> Recorder::Impl::init(const RenderSettings& settings) {
    ...
    // Use avcodec_find_encoder_by_name directly — no need for a custom iterator
    ...
    if (settings.m_hardwareAccelerationType != HardwareAccelerationType::NONE &&
        (ret = av_hwdevice_ctx_create(&m_hwDevice, (AVHWDeviceType)settings.m_hardwareAccelerationType, NULL, NULL, 0)) < 0)
    ...
    // Pick a pixel format from the codec's supported list
    if (const AVPixelFormat* pix_fmt = m_codec->pix_fmts) {
        while (*pix_fmt != AV_PIX_FMT_NONE) {
            if (*pix_fmt == AV_PIX_FMT_MEDIACODEC) {
                // MediaCodec surface mode expects a surface; force NV12 instead
    ...
    // m_frame always holds the settings pixel format; conversion to codec format happens in writeFrame
    ...
    if ((AVPixelFormat)settings.m_pixelFormat != m_codecContext->pix_fmt) {
        m_swsCtx = sws_getContext(
            m_codecContext->width, m_codecContext->height, (AVPixelFormat)settings.m_pixelFormat,
            m_codecContext->width, m_codecContext->height, m_codecContext->pix_fmt,
...
void Recorder::Impl::stop() {
    ...
    // Free av_image_alloc'd raw buffers before freeing the frames
    if (m_frame) {
    ...
    // filteredFrame is always allocated if init succeeded; free unconditionally
    if (m_filteredFrame)
